    ...
    public class MapDataTests
    {
      ...
      [SetUp]
        ...
        public IEnumerator ShuffleLocationsTest_Expect_ShuffledLocations()
        {
            //Arrange 
            for (int i = 0; i < 5; i++)
                mapData.AddNewLocation(new Location { SpawnChance = 100, MapData = placeholder, MainTeleportTrigger = placeholder.GetComponent<TeleportTrigger>() });
            //Act
        ...
        public IEnumerator ShuffleLocationsTest_Compare_Multithreading_To_OneThread_Expect_MultithreadingFaster_1000()
        {
            ...
            for (int i = 0; i < 1000; i++)
                mapData.AddNewLocation(new Location { SpawnChance = 100, MapData = placeholder, MainTeleportTrigger = placeholder.GetComponent<TeleportTrigger>() });
            //Act
            mapData.enableAutomaticallyIfGreater = -1;
            stopwatch.Start();
            mapData.ShuffleLocations();
            stopwatch.Stop();
            yield return new WaitForSeconds(0.5f);
