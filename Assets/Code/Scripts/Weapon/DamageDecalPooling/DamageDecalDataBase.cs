...
namespace WeaponNS.ShootingWeaponNS
{
	...
	public class DamageDecalDataBase : MonoBehaviour
	{
		...
		protected DamageDecalObject[] DefineAvailableDecalsByInfo(RaycastHit hitInfo, WeaponEntity weaponType)
		{
			...
			if (damageDecals == null || damageDecals.Length == 0)
				damageDecals = new DamageDecalObject[1] { new DamageDecalObject { DamageDecalType = WeaponConstants.DEFAULT_DAMAGE_DECAL } };
			if (damageDecals.Length > 1)
				damageDecals = damageDecals.Where(x => x.DamageDecalType != WeaponConstants.DEFAULT_DAMAGE_DECAL).ToArray();
			return damageDecals;
		}
