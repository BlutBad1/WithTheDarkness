...
using EntityNS.Base;
using EntityNS.Base.StateBehaviourNS.Priority;
...
namespace EntityNS.Type.Gramophone.Attack
{
    public class GramophoneSphereAttack : EntityStateAttack
    {
        [SerializeField, FormerlySerializedAs("SphereColliderAttack")]
        private SphereCollider sphereColliderAttack;
        [SerializeField]
        private EntitySpotTarget enemySpotTarget;
        [SerializeField]
        private EntityCreature enemyCreature;
        ...
        {
            bool result = (enemySpotTarget.KnownCreatures[opponent].GetCreatureType() != enemyCreature.GetCreatureType());
            if (result)
        ...
        private void SetGameObjectSpeed(ICreature creature, float speed) =>
            creature.CurrentSpeedCoefficient = speed;
