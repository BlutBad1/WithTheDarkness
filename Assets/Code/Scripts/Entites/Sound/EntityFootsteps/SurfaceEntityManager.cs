...
namespace EntityNS.Sound
{
    ...
    [System.Serializable]
    public struct EnemyRegisteredMaterial
    {
        public Texture texture;
        public int surfaceIndex;
        [HideInInspector]
        public string[] attachedSurfaces;
    }
    public class SurfaceEntityManager : MonoBehaviour
    {
        [SerializeField] EnemySurfaceDefinition[] definedSurfaces;
        ...
        {
            if (registeredTextures != null && registeredTextures.Length > 0)
                for (int i = 0; i < registeredTextures.Length; i++)
                    registeredTextures[i].attachedSurfaces = GetAllSurfaceNames();

        }

        public Dictionary<AudioClip, float> GetFootstep(Collider groundCollider, Vector3 worldPosition)
        {
            int surfaceIndex = GetSurfaceIndex(groundCollider, worldPosition);
            if (surfaceIndex == -1)
                return null;
            // Getting the footstep sounds based on surface index.
            EnemySurfaceDefinition surface = definedSurfaces[surfaceIndex];
            AudioClip[] footsteps = surface.footsteps;
            n = Random.Range(footsteps.Length > 1 ? 1 : 0, footsteps.Length);
            // Move picked sound to index 0 so it's not picked next time.
            AudioClip temp = footsteps[n];
            footsteps[n] = footsteps[0];
            footsteps[0] = temp;
            return new Dictionary<AudioClip, float>() { { temp, surface.volume } };
        }
        public Dictionary<AudioClip, float> GetLandsound(Collider groundCollider, Vector3 worldPosition)
        {
            int surfaceIndex = GetSurfaceIndex(groundCollider, worldPosition);
            if (surfaceIndex == -1)
                return null;
            // Getting the footstep sounds based on surface index.
            EnemySurfaceDefinition surface = definedSurfaces[surfaceIndex];
            AudioClip[] footsteps = surface.landsounds;
