...
namespace PortalNS
{
    public class Portal : MonoBehaviour
    {
        ...
        public void PortalLight()
        {
            if (FindObjectsOfType<PlayerPortalTraveller>().Any(x => x.FromSideOfPortal == this))
            {
        ...
        public void Render()
        {
            // Skip rendering the view from this portal if the player is not looking at the linked portal
            if (!IsPortalVisible())
                return;
            ...
            for (int i = 0; i < recursionLimit; i++)
            {
                if (i > 0)
                {
                    // No need for recursive rendering if the linked portal is not visible through this portal
            ...
            // Hide the screen so that the camera can see through the portal
            ...
            // Unhide objects hidden at the start of the render
        ...
        public List<PortalTraveller> GetTrackedPortalTravellers() => trackedTravellers;
         ...
         */
        public bool IsPortalVisible() =>
            CameraUtility.VisibleFromCamera(linkedPortal.screen, playerCam);
